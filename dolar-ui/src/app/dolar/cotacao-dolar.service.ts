...
export class CotacaoDolarService {
  ...
  obterCotacaoDolarAtual(): Promise<CotacaoDolar> {
    return this.httpClient.get<CotacaoDolarDTO>(`${this.apiUrl}/dolar/atual`)
      .toPromise()
      .then((cotacaoDTO: CotacaoDolarDTO) => this.converterCotacaoDolarDTOParaCotacaoDolar(cotacaoDTO));
  }
   ...
   */
  listarCotacoesDolarNoPeriodo(filtro: CotacaoDolarFiltro): Promise<Page<CotacaoDolar>> {
    const params = this.construirParametrosDeRequisicao(filtro);
    return this.httpClient.get<Page<CotacaoDolarDTO>>(`${this.apiUrl}/dolar`, { params })
      .toPromise()
      .then((pageDTO: Page<CotacaoDolarDTO>) => this.converterPageCotacaoDolarDTOParaPageCotacaoDolar(pageDTO));
  }
   ...
   * @returns 
   */
  private converterCotacaoDolarDTOParaCotacaoDolar(cotacaoDTO: CotacaoDolarDTO): CotacaoDolar {
    return {
      valor: Number(cotacaoDTO.valor),
      data: DateTime.fromFormat(cotacaoDTO.data, 'dd/MM/yyyy - hh:mm:ss').toJSDate(),
   ...
   */
  private converterPageCotacaoDolarDTOParaPageCotacaoDolar(pageDTO: Page<CotacaoDolarDTO>): Page<CotacaoDolar> {
   const conteudo = pageDTO.content.map(this.converterCotacaoDolarDTOParaCotacaoDolar);
    return {
      content: conteudo,
   ...
   */
  private construirParametrosDeRequisicao(filtro: CotacaoDolarFiltro): HttpParams {
    return new HttpParams()
    .append('data-inicial', DateTime.fromJSDate(filtro.dataInicial).toFormat('yyyy-MM-dd'))
    .append('data-final', DateTime.fromJSDate(filtro.dataFinal).toFormat('yyyy-MM-dd'))
