import { CotacaoDolarFiltro, CotacaoDolarService } from '../cotacao-dolar.service';
...
@Component({
  ...
  styleUrls: ['./listagem-cotacao-dolar.component.css']
})
export class ListagemCotacaoDolarComponent{
   ...
   */
  @debounce()
  pesquisarComLazyLoad(evento: LazyLoadEvent): void{
    const numeroDaPagina = (evento.first && evento.rows) ? (evento.first / evento.rows) : 0;
    this.carregandoRegistros = true;

    if(this.cotacaoDolarFiltro == null)
      throw new Error('Erro ao realizar lazy load da tabela de cotações por período. O filtro está nulo.');
   ...
   */
  isCampoInvalido(campo: FormControl): boolean {
    if(campo){
  ...
  private pesquisarCotacaoDoDolarNoPeriodo(filtro: CotacaoDolarFiltro) {
    this.cotacaoDolarService.listarCotacoesDolarNoPeriodo(filtro)
